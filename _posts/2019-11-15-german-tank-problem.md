...
In general, an *estimator* maps an outcome of a random experiment to an estimate of a parameter that characterizes the dgp. In the German tank problem, an outcome is a set of serial numbers on the captured tanks, $$\{x_1, x_2, ..., x_k\}$$, and we aim to, from this, estimate the total number of tanks, $n$, which fully characterizes the dgp (in addition to $k$). One estimator of $n$ for the German tank problem is the maximum observed serial number $m\equiv \max x_i$. 
